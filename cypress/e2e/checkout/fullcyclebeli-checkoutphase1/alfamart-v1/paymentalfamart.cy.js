...
describe('Should Success When Checkout Payment with Alfamart', () => {
  const checkoutPayment = () => {
    cy.log('Waitting Alfamart')
    cy.wait(6000)
    cy.get('body')
      .then($body => {
        let possibleBtnBankTransfer = ['btnAlfamart', 'btnLastAlfamart'];
        if ($body.find('div[data-testid="btnAlfamart"]').length) {
            return 'div[data-testid="btnAlfamart"]'
        }
    
        if ($body.find('div[data-testid="btnLastAlfamart"]').length) {
          return 'div[data-testid="btnLastAlfamart"]'
        }
        return null;
      }).then(selector => {
    ...
    // cy.screenshot('alfamart-selesaikanpembayaran');
    cy.get('div')
    ...
    cy.get('[class="css-901oao r-ba7s3 r-1inkyih r-16dba41 r-3ucsff r-7ptqe7 r-ku1wi2 r-1joea0r"]')
      .should('be.visible')
    // Cypress.Screenshot.defaults({ capture: 'runner' });
    // cy.screenshot('alfamart-orderdetail');
  };
...
//END POSITIVE CASE : Should Success When Checkout Payment with Alfamart
