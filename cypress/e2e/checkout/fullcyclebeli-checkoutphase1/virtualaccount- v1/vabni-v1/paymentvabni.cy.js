...
describe('Should Success When Checkout Payment with VA BNI', () => {
  const checkoutPayment = () => {
    ...
    cy.get('body')
      .then($body => {
        let possibleBtnBankTransfer = ['radPaymentVABNI', 'radLastPaymentVABNI'];
        if ($body.find('div[data-testid="radPaymentVABNI"]').length) {
            return 'div[data-testid="radPaymentVABNI"]'
        }
    
        if ($body.find('div[data-testid="radLastPaymentVABNI"]').length) {
          return 'div[data-testid="radLastPaymentVABNI"]'
        }
        return null;
      }).then(selector => {
      cy.log('selector', selector);
    ...
    // cy.screenshot('vabni-selesaikanpembayaran');
    cy.get('div')
    ...
    cy.get('[class="css-901oao r-ba7s3 r-1inkyih r-16dba41 r-3ucsff r-7ptqe7 r-ku1wi2 r-1joea0r"]')
      .should('be.visible')
    // Cypress.Screenshot.defaults({ capture: 'runner' });
    // cy.screenshot('vabni-orderdetail');
  };
...
//END POSITIVE CASE : Should Success When Checkout Payment with VA BNI
