...
describe('Should Success When Checkout Payment with VA Bank Lainnya', () => {
  const checkoutPayment = () => {
    ...
    cy.get('body')
      .then($body => {
        let possibleBtnBankTransfer = ['radPaymentVAOtherBank', 'radLastPaymentVAOtherBank'];
        if ($body.find('div[data-testid="radPaymentVAOtherBank"]').length) {
            return 'div[data-testid="radPaymentVAOtherBank"]'
        }
    
        if ($body.find('div[data-testid="radLastPaymentVAOtherBank"]').length) {
          return 'div[data-testid="radLastPaymentVAOtherBank"]'
        }
        return null;
      }).then(selector => {
      cy.log('selector', selector);
    ...
    // cy.screenshot('vabanklainnya-selesaikanpembayaran');
    cy.get('div')
    ...
    cy.get('[class="css-901oao r-ba7s3 r-1inkyih r-16dba41 r-3ucsff r-7ptqe7 r-ku1wi2 r-1joea0r"]')
      .should('be.visible')
    // Cypress.Screenshot.defaults({ capture: 'runner' });
    // cy.screenshot('vabanklainnya-orderdetail');
  };
...
//END POSITIVE CASE : Should Success When Checkout Payment with VA Bank Lainnya
