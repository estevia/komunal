...
describe('Should Success When Checkout Payment with VA BRI', () => {
  const checkoutPayment = () => {
    ...
    cy.get('body')
      .then($body => {
        let possibleBtnBankTransfer = ['radPaymentVABRI', 'radLastPaymentVABRI'];
        if ($body.find('div[data-testid="radPaymentVABRI"]').length) {
            return 'div[data-testid="radPaymentVABRI"]'
        }
    
        if ($body.find('div[data-testid="radLastPaymentVABRI"]').length) {
          return 'div[data-testid="radLastPaymentVABRI"]'
        }
        return null;
      }).then(selector => {
      cy.log('selector', selector);
    ...
    // cy.screenshot('vabri-selesaikanpembayaran');
    cy.get('div')
    ...
    cy.get('[class="css-901oao r-ba7s3 r-1inkyih r-16dba41 r-3ucsff r-7ptqe7 r-ku1wi2 r-1joea0r"]')
      .should('be.visible')
    // Cypress.Screenshot.defaults({ capture: 'runner' });
    // cy.screenshot('vabri-orderdetail');
  };
...
//END POSITIVE CASE : Should Success When Checkout Payment with VA BRI
